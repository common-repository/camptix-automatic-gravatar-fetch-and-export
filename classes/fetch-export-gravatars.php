...
class CampTix_Addon_Gravatar_Fetch extends CampTix_Addon {
	...
	function add_to_default_addon( $default_addons ) {
		return array_merge( $default_addons, array(
			'fetch-export-gravatars' => FETCH_GRAVATARS_PLUGIN_URL . "classes/fetch-export-gravatars.php", 
		) );
	}
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td><label><input type="checkbox" name="tix_export_include_gravatars" value="1" checked="checked"/> <?php _e('Include Gravatars' , 'camptix'); ?></label></td>
					</tr>
					<tr>
	...
	function generate_attendee_report_with_gravatars( $format , $selected_columns , $selected_questions ) {
		...
		if ( !isset( $_POST['tix_export_include_gravatars'] ) ) {
			...
			$this->zip_Files_and_Download( $files_in_gravatar_dir , $filename_zip , $gravatar_dir );
